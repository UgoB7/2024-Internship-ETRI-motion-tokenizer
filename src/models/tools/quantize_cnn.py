...
class QuantizeEMAReset(nn.Module):
    ...
    def _tile(self, x):
        ...
        # print('###############  x.shape tile  #######################', x.shape) # torch.Size([4096, 512])
    ...
    def quantize(self, x):
        # print('###############  x.shape  #######################', x.shape)  # torch.Size([4096, 512])
    ...
    def forward(self, x):
        N, width, T = x.shape
        # print('###############  N, width, T  #######################', N, width, T)  # 256 512 16

        # Preprocess
        x = self.preprocess(x)
        # NCT -> NTC -> [NT, C] # torch.Size([4096, 512])
        ...
        # print('###############  code_idx.shape  #######################', code_idx.shape)  # torch.Size([4096])
        x_d = self.dequantize(code_idx)
        # print('###############  x_d.shape  #######################', x_d.shape)  # torch.Size([4096, 512])

        # Update embeddings
        if self.training:
            perplexity = self.update_codebook(x, code_idx)
        else : 
            perplexity = self.compute_perplexity(code_idx)  # perplexity = torch.exp(-torch.sum(prob * torch.log(prob + 1e-7)))
