...
class JointSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.selected_channels = list(dict.fromkeys(selected_channels))  # remove duplicates
...
class RootNormalizer(BaseEstimator, TransformerMixin):
    """
    Make mean root position and rotation to be zero
    ...
    def transform(self, X, y=None):
        ...
        for track in X:
            ...
            # all_zeros = np.zeros(track.values[xp_col].values.shape)
            # mean_xr = np.mean(track.values[xr_col].values)
            # mean_yr = np.mean(track.values[yr_col].values)
            # mean_zr = np.mean(track.values[zr_col].values)
            # new_df[xr_col] = pd.Series(data=track.values[xr_col]-mean_xr, index=new_df.index)
            # new_df[yr_col] = pd.Series(data=track.values[yr_col]-mean_yr, index=new_df.index)
            # new_df[zr_col] = pd.Series(data=track.values[zr_col]-mean_zr, index=new_df.index)
